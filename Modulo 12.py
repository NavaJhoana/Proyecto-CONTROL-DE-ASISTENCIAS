...
class HistorialFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.tree_frame = tk.Frame(self, bg=COLORS["bg_main"])
    ...
    def mostrar_historial(self):
        ...
        info_frame = tk.LabelFrame(self.tree_frame, text="Información del Empleado", font=FONT_NORMAL, bg=COLORS["bg_card"])
        ...
        campos = [
            ...
            ("Grado de Estudio", personal.get("ultimo_grado_estudio", "")),
            ("Cédula Profesional", personal.get("cedula_profesional", "")),
            ("Domicilio", personal.get("domicilio", "")),
            ("Teléfono", personal.get("telefono", "")),
            ("Correo", personal.get("correo_electronico", "")),
            ("Fecha de Ingreso", personal.get("fecha_ingreso", "")),
            ("Vacaciones", personal.get("vacaciones", "No asignadas")),
            ("Turno", personal.get("turno_asignado_nombre", "No asignado")),
            ("Horario", f"{personal.get('horario_inicio', '-')}-{personal.get('horario_fin', '-')}")
        ]

        for i, (campo, valor) in enumerate(campos):
            tk.Label(info_frame, text=f"{campo}:", font=("Segoe UI", 10, "bold"), bg=COLORS["bg_card"]).grid(row=i, column=0, sticky="e", padx=5, pady=1)
            tk.Label(info_frame, text=valor, font=("Segoe UI", 10), bg=COLORS["bg_card"]).grid(row=i, column=1, sticky="w", padx=5, pady=1)

        tabla = ttk.Treeview(self.tree_frame, columns=("Fecha", "Entrada", "Salida", "Retardo", "Extra"), show="headings", height=15)
        ...
        fechas_ordenadas = sorted(fechas_registradas, key=lambda x: datetime.strptime(x, "%d/%m/%Y"), reverse=True)
        ...
        for fecha in fechas_ordenadas:
            ...
            if fecha in retardos_registrados.get(id_, {}):
                info = retardos_registrados[id_][fecha]
                retardo = f"{info['tipo_ultimo']} ({info['hora_registro']})"
            tabla.insert("", "end", values=(fecha, entrada, salida, retardo, f"{extra} min" if extra else "—"))
