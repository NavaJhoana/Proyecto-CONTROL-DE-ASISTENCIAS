...
class VacacionesFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        form_frame.place(relx=0.5, rely=0.2, anchor="n", width=750, height=500)

        tk.Label(form_frame, text="ID del trabajador:", font=FONT_NORMAL, bg=COLORS["bg_card"]).place(relx=0.05, rely=0.05)
        ...
        bloque1_frame = tk.LabelFrame(form_frame, text="Enero - Julio", font=FONT_NORMAL, bg=COLORS["bg_card"])
        bloque1_frame.place(relx=0.05, rely=0.2, relwidth=0.4, relheight=0.5)

        bloque2_frame = tk.LabelFrame(form_frame, text="Julio - Diciembre", font=FONT_NORMAL, bg=COLORS["bg_card"])
        bloque2_frame.place(relx=0.5, rely=0.2, relwidth=0.4, relheight=0.5)
        ...
        for p in self.periodos_bloque1:
            tk.Radiobutton(bloque1_frame, text=p, variable=self.bloque1_var, value=p, anchor="w",
                           font=("Segoe UI", 9), bg=COLORS["bg_card"]).pack(anchor="w")

        for p in self.periodos_bloque2:
            tk.Radiobutton(bloque2_frame, text=p, variable=self.bloque2_var, value=p, anchor="w",
                           font=("Segoe UI", 9), bg=COLORS["bg_card"]).pack(anchor="w")
...
class HistorialFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.text_historial.pack(padx=20, pady=10, fill='both', expand=True)
    ...
    def mostrar_historial(self):
        ...
        fechas_registradas.update(retardos_registrados.get(id_, {}).keys()) 

        fechas_ordenadas = sorted(list(fechas_registradas), key=lambda x: datetime.strptime(x, "%d/%m/%Y"), reverse=True)

        turno_info = "No asignado"
        horario_info = "N/A"
        dias_laborales_info = "N/A"
        if personal.get("turno_asignado_nombre"):
            turno_info = personal["turno_asignado_nombre"]
            horario_info = f"{personal['horario_inicio']} - {personal['horario_fin']}"
            dias_laborales_info = ", ".join(personal["dias_laborales_turno"])

        menores_acumulados = retardos_acumulados_quincena.get(id_, {}).get('menores', 0)
        mayores_acumulados = retardos_acumulados_quincena.get(id_, {}).get('mayores', 0)
        ultima_evaluacion_fecha = retardos_acumulados_quincena.get(id_, {}).get('ultima_fecha_evaluada', None)
        ultima_evaluacion_str = ultima_evaluacion_fecha.strftime('%d/%m/%Y') if isinstance(ultima_evaluacion_fecha, date) else 'N/A'
...
Turno asignado: {turno_info} ({horario_info})
Días laborales: {dias_laborales_info}
-----------------------------

--- Retardos Acumulados (Quincena Actual) ---
Menores: {menores_acumulados}/{LIMITE_RETARDOS_MENORES}
Mayores: {mayores_acumulados}/{LIMITE_RETARDOS_MAYORES}
Última evaluación de quincena: {ultima_evaluacion_str}
-----------------------------

--- Penalizaciones ---
"""
        if penalizaciones_aplicadas.get(id_):
            for p in penalizaciones_aplicadas[id_]:
                texto += f"  - Fecha: {p['fecha']}, Tipo: {p['tipo']}, Razón: {p['razon']}\n"
        else:
            texto += "  No hay penalizaciones registradas.\n"

        texto += "\n--- Historial Diario ---\n"
        if fechas_ordenadas:
            for fecha in fechas_ordenadas:
                entrada_hora = asistencias_registradas.get(id_, {}).get(fecha, 'No registrada')
                salida_hora = salidas_registradas.get(id_, {}).get(fecha, 'No registrada')
                tiempo_extra = horas_extra_registradas.get(id_, {}).get(fecha, 0)

                retardo_info = "No"
                if fecha in retardos_registrados.get(id_, {}):
                    tipo_retardo_dia = retardos_registrados[id_][fecha].get("tipo_ultimo")
                    hora_retardo = retardos_registrados[id_][fecha].get("hora_registro")
                    if tipo_retardo_dia:
                        retardo_info = f"Sí ({tipo_retardo_dia} a las {hora_retardo})"


                texto += f"  Fecha: {fecha}\n"
                texto += f"    Entrada: {entrada_hora}\n"
                texto += f"    Salida: {salida_hora}\n"
                texto += f"    Retardo: {retardo_info}\n"
                if tiempo_extra > 0:
                    texto += f"    Tiempo Extra: {tiempo_extra} minutos\n"
                texto += "\n"
        else:
            texto += "  No hay registros de asistencia/salida para este ID.\n"

        self.text_historial.insert("1.0", texto)
